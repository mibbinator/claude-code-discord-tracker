...
- **Usage tracker** (`notify-discord-usage.{ps1,sh}`) — fires on `PostToolUse`(matcher `*`) + `Stop`; posts only when `floor(utilization)` rises for the 5h or weekly window (~every 1%), OR when a window resets. Silent for routine 1% ticks (combined "Usage update" embed); @-mentions the user (a) when a crossing passes a milestone (`$Milestones`/`MILESTONES` = 25/50/80/90/100), or (b) on a **reset** — detected as `cur% < prev%` (each window accumulates monotonically then drops to ~0 at its boundary). Resets post one **separate** embed **per window** (5h and weekly never combined): a bright-green 🔄 "…LIMIT RESET" for a scheduled rollover, or a gold ⚡ "ANTHROPIC RESET…" when the drop happens while the **previously-stored `resets_at` is still >5min in the future** (= Anthropic cleared the limit early for everyone). State (`~/.claude/discord_usage_pct_state.json`) persists per-window `{five_hour, seven_day, five_hour_resets_at, seven_day_resets_at}` — the reset times are what enable manual-vs-scheduled detection (first run baselines silently). Webhook from `~/.claude/discord_usage_webhook.txt`. Reuses the ping feed's usage-fetch/OAuth/bar/reset-in logic — keep them in sync.
